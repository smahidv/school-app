...
export default function ExamTable({moduleId,classId}) {

  const [exams, setexams] = useState([]);


  function getClassExams(){
    axiosClient.get(`/class-exams?moduleId=${moduleId}&classRoomId=${classId}`).then(({data})=>{
    ...
    <div className="relative flex-[1_auto] flex flex-col break-words min-w-0 bg-clip-border rounded-[.95rem] bg-white m-5">
      <div className="relative flex flex-col min-w-0 break-words border border-dashed bg-clip-border rounded-2xl border-stone-200 bg-light/30">
          ...
          <h3 className="px-9 pt-5  flex flex-col items-start justify-center m-2 ml-0 font-medium text-xl/tight text-dark">
            <span className="mr-3 font-semibold text-dark">Exams</span>
            <span className="mt-1 font-medium text-secondary-dark text-lg/normal">All exams from </span>
          </h3>
